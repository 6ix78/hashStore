...
exports.getDepositAddress = async (req, res) => {
  ...
  if (!userAddress) {
    userAddress = (await COINPAYMENT.getDepositAddress(currency.toUpperCase()))
      .address;
...
exports.getDepositAddressForAsicContarct = async (req, res) => {
  ...
  let asicContractAddress = (
    await COINPAYMENT.getDepositAddressForAsicContract(currency)
  ).address;
...
exports.verifyNotification = (req, res, next) => {
  const { MERCHANT_ID, IPN_SECRET } = process.env;
  if (
    !req.get(`HMAC`) ||
    !req.body.ipn_mode ||
...
exports.depositNotification = async (req, res) => {
  ...
  if (status === '100')
    await mUser.UpdateBalance(userID, currency, amount - amount * 0.005);
  if (!deposit) {
...
exports.depositNotificationForAsicContract = async (req, res) => {
  ...
  if (status === '100') {
    ...
    //===================
    const planContracts = await mPlanContarct.getWorkerPlanContracts(
      asicContract._id
    );
    if (planContracts[0]) {
      await planContracts.forEach(async (c) => {
        if (+c.endDate < Date.now()) {
          ...
          await mSeller.updateSellerWorkerHashRate(
            c.sellerWorkerID,
            c.hashPower
          );
          return;
...
exports.setWithdrawRequest = async (req, res) => {
  ...
  if (amount <= 0 || !currency || !amount || !address)
  ...
  switch (coin) {
    case 'ETH':
      if (amount > user.balance.eth)
        return res.status(400).json({ message: 'no sufficient balance' });
      break;
    case 'BTC':
      ...
      break;
    case 'LTCT':
      if (amount > user.balance.ltct)
        return res.status(400).json({ message: 'no sufficient balance' });
      break;
    case 'RVN':
      if (amount > user.balance.rvn)
        return res.status(400).json({ message: 'no sufficient balance' });
      break;
    default:
